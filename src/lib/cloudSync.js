...
// Wraps loadCloudState with a single retry before ever trusting a
// 'not_found' result. Right after a page refresh, the Supabase session/auth
// token can still be attaching — a query that runs during that tiny window
// can come back with zero rows (which looks identical to "no data exists")
// even though the row is really there. Retrying once, after explicitly
// re-confirming the session, closes that race without slowing down the
// normal case (a real 'ok' or a real 'error' returns immediately, no retry).
export async function loadCloudStateWithRetry(userId) {
  const first = await loadCloudState(userId)
  if (first.status !== 'not_found') return first
  try { await supabase.auth.getSession() } catch { /* ignore */ }
  await new Promise(resolve => setTimeout(resolve, 400))
  return loadCloudState(userId)
}
